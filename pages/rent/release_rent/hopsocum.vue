	...
	<view>
		<view class="releabBox">
			...
			<view class="releaitianjaBox">
				<view class="releaitianja" @click="addMore"><text>添加更多证件照</text>
		...
		onLoad(e) {
			...
			for (var i in transferShopInfo.paperwork_img) {
				for (var j in transferShopInfo.paperwork_img[i]) {
					var arr = {
						src: transferShopInfo.paperwork_img[i][j].img_url
					};
					if (uploadImages[i]) {
					...
					if (me.imgs[i][j]) {
						...
						me.imgs[i][j] = {
							type: i,
							title: transferShopInfo.paperwork_img[i][j].type_name,
							show_img: transferShopInfo.paperwork_img[i][j].show_img_url
						};
					}
				}
			}
