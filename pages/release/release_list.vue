		...
		<view class="findcontBox">
			<view v-show="current === 0">
				<view v-if="transferShopData.length > 0">
					<view class="findconnerBox" v-for="(item,index) in transferShopData" :key="index">
						<view class="findconnerhz">
							<navigator v-if="item.status == 1" class="fufeizd" :url="'../user/order_pay?type=1&info_id='+item.id">推广置顶</navigator>
							<navigator class="findcerhzimgrm" :url="'../transfer_shop/transfer_shop_detail?id='+item.id">
							...
							<view class="findconnerhztx">
								...
								<view class="dconnerhcazuo">
									<view class="nerhcazuolyan">
										...
										<view class="nerhcajjolyantx" style="color:#333333;">转让费: {{item.unit_transfer_price}}</view>
									</view>
								...
								<view class="dconnerhyouhdf">
									...
									<view class="dconnerquxao" v-on:click="releaseDel(item.id)">删除</view>
								</view>
							</view>
						</view>
					</view>
				...
				<view style="text-align: center;padding-top:150upx;" v-else>
					...
					<view style="color:#888888;">暂无转租信息</view>
				</view>
			</view>
			<view v-show="current === 1">
				...
				<view style="text-align: center;padding-top:150upx;" v-else>
					<image src="../../static/images/zpzp_95.png" mode="" style="width:300upx;height:300upx;"></image>
					<view style="color:#888888;">暂无发布信息</view>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			releaseDel(id) {
				...
				if (_this.current == 0) {
					url = _this.$api.transferShop.releaseDel;
				} else {
					url = _this.$api.transferShop.releaseSeekDel;
				}
			...
			getTransferShopInfo(isReload) {
				...
				_this.$Request.get(_this.$api.transferShop.releaseList, {
					page: page,
					limit: limit,
					type: type,
				}).then(res => {
