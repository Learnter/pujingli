<template>
	<view>
		<view>
			<view class="smaNavBoxpos">
				<view class="smaNavBoxpohe">
					<view class="smaNavBox">
						<view class="smaNav" @click="togglePopup('bottomd')">行业 <uni-icon class="i" type="arrowdown" color="#333333" size="18"></uni-icon>
						</view>
						<view class="smaNav" @click="togglePopup('bottoma')">区域 <uni-icon class="i" type="arrowdown" color="#333333" size="18"></uni-icon>
						</view>
						<view class="smaNav" @click="togglePopup('bottomb')">面积 <uni-icon class="i" type="arrowdown" color="#333333" size="18"></uni-icon>
						</view>
						<view class="smaNav" @click="togglePopup('bottomc')">费用 <uni-icon class="i" type="arrowdown" color="#333333" size="18"></uni-icon>
						</view>
					</view>
				</view>
			</view>
			<view>
        
        
			<uni-popup :show="type === 'bottoma'" position="bottom" mode="fixed" msg="区域" @hidePopup="togglePopup('')">
					<view class="dxxunatBox">
						<!-- <view class="dxxunButn">确认</view> -->
						<view class="xunanNenr clearfix">
							<scroll-view scroll-y="true" class='xunanLeftBocx'>
								<view class="xunanLeft">
									<!-- <view class="xufltext">全部</view> -->
									<view class="xufltext" @click="regionChoose(1, item)" :class="item.id == district ? 'xufltextActive' : ''"
									 v-for="(item, index) in region.child" :key="index">{{item.name}}</view>
								</view>
							</scroll-view>
							<scroll-view scroll-y="true" class='xunanRightBox'>
								<view class="xunanRight" v-if="region.child[district].id > 0">
									<view class="xufltext" @click="regionChoose(2, item)" :class="item.id == twon ? 'xufltextActive' : ''" v-for="(item, index) in region.child[district].child"
									 :key="index">
										<text>{{item.name}}</text>
										<uni-icon class="i" type="checkmarkempty" color="#f000000" size="18"></uni-icon>
									</view>
								</view>
								<view class="xunanRight" v-else>
									<view class='xufltext xufltextActive'>
										<text>不限</text>
										<uni-icon class="i" type="checkmarkempty" color="#f000000" size="18"></uni-icon>
									</view>
								</view>
							</scroll-view>
						</view>
					</view>
				</uni-popup>
        
        
				<uni-popup :show="type === 'bottomb'" position="bottom" mode="fixed" msg="面积范围" @hidePopup="togglePopup('')">
					<view class="xunanCont">
						<!-- <view class="dxxunButn">确认</view> -->
						<view class="xunantextBox clearfix">
							<view class="xunantexthe" v-for="(item,index) in areaData" :key="index">
								<view class="xunantext" @click="areaChoose(item)" :class="item.id == choseeAreaInfo.id ? 'xunantextActive' : ''"
								 v-if="item.min <= 0 && item.max <= 0">不限</view>
								<view class="xunantext" @click="areaChoose(item)" :class="item.id == choseeAreaInfo.id ? 'xunantextActive' : ''"
								 v-if="item.min <= 0 && item.max > 0">{{item.max}}㎡以下</view>
								<view class="xunantext" @click="areaChoose(item)" :class="item.id == choseeAreaInfo.id ? 'xunantextActive' : ''"
								 v-if="item.min > 0 && item.max <= 0">{{item.min}}㎡以上</view>
								<view class="xunantext" @click="areaChoose(item)" :class="item.id == choseeAreaInfo.id ? 'xunantextActive' : ''"
								 v-if="item.min > 0 && item.max > 0">{{item.min}}-{{item.max}}㎡</view>
							</view>
						</view>
					</view>
				</uni-popup>
        
        
				<uni-popup :show="type === 'bottomc'" position="bottom" mode="fixed" msg="租金" @hidePopup="togglePopup('')">
					<view class="xunanCont">
						<!-- <view class="dxxunButn">确认</view> -->
						<view class="xunantextBox clearfix">
							<view class="xunantexthe" v-for="(item,index) in rentData" :key="index">
								<view class="xunantext" @click="rentChoose(item)" :class="choseeRentInfo.id == item.id ? 'xunantextActive' : ''">{{item.title}}</view>
							</view>
						</view>
					</view>
				</uni-popup>
        
				<uni-popup :show="type === 'bottomd'" position="bottom" mode="fixed" msg="行业" @hidePopup="togglePopup('')">
					<view class="dxxunatBox">
						<!-- <view class="dxxunButn">确认</view> -->
						<view class="xunanNenr clearfix">
							<scroll-view scroll-y="true" class='xunanLeftBocx'>
								<view class="xunanLeft">
									<view class="xufltext" @click="cateChoose(1, item)" :class="item.id == topCateId ? 'xufltextActive' : ''"
									 v-for="(item,index) in industryCate" :key="index">{{item.name}}</view>
								</view>
							</scroll-view>
							<scroll-view scroll-y="true" class='xunanRightBox'>
								<view class="xunanRight" v-if="industryCateChild.length > 0">
									<view class="xufltext" @click="cateChoose(2, item)" :class="item.id == choseeCateInfo.id ? 'xufltextActive' : ''"
									 v-for="(item, index) in industryCateChild" :key="index">
										<text>{{item.name}}</text>
										<uni-icon class="i" type="checkmarkempty" color="#f000000" size="18"></uni-icon>
									</view>
								</view>
								<view class="xunanRight" v-else>
									<view class='xufltext xufltextActive'>
										<text>不限</text>
										<uni-icon class="i" type="checkmarkempty" color="#f000000" size="18"></uni-icon>
									</view>
								</view>
							</scroll-view>
						</view>
					</view>
				</uni-popup>
			</view>
		</view>
		<view v-if="seekingData.length > 0">
			<navigator class="qnquNavBox" :url="'../transfer_shop/seeking_detail?id='+item.id" v-for="(item,index) in seekingData"
			 :key="index">
				<view class="qnquNavtl">
					<view class="qnquNavtltx">{{item.title}}</view>
					<view class="qnquNavtime">{{item.add_time}}</view>
				</view>
				<view class="qnquCont" v-if="item.address.twon">
					<view class="qnquConttl">优先选择：</view>
					<view class="qnquConttex_1">{{item.address.twon}}</view>
				</view>
				<view class="qnquCont">
					<view class="qnquConttl">求租面积：</view>
					<view class="qnquConttex_1">{{item.area_add}}~{{item.area_end}}㎡</view>
				</view>
				<view class="qnquCont">
					<view class="qnquConttl">租金预算：</view>
					<view class="qnquConttex_2">{{item.rent_add}}~{{item.rent_end}}元/月</view>
				</view>
			</navigator>
			<view class="uni-loadmore" v-if="showLoadMore">{{loadMoreText}}</view>
		</view>

		<!-- 暂无数据 s -->
		<view style="text-align: center;padding-top:150upx;" v-else>
			<image src="../../static/images/zpzp_95.png" mode="" style="width:300upx;height:300upx;"></image>
			<view style="color:#888888;">暂无求租信息</view>
		</view>
		<!-- 暂无数据 e -->
	</view>
</template>

<script>
	import uniIcon from '@/components/uni-icon/uni-icon.vue';
	import uniPopup from '@/components/uni-popup/uni-popup.vue';
	let city = 0,
		page = 0,
		limit = 10,
		is_load_more = true,
		webConfig = {};
	export default {
		data() {
			return {
				type: '',
				region: {},
				district: 0,
				twon: 0,
				topCateId: 0,
				industryCate: [],
				industryCateChild: [],
				choseeCateInfo: {},
				choseeAreaInfo: {
					id: 1,
					min: 0,
					max: 0
				},
				areaData: [{
						id: 1,
						min: 0,
						max: 0
					},
					{
						id: 2,
						min: 0,
						max: 20
					},
					{
						id: 3,
						min: 20,
						max: 50
					},
					{
						id: 4,
						min: 50,
						max: 100
					},
					{
						id: 5,
						min: 100,
						max: 200
					},
					{
						id: 6,
						min: 200,
						max: 500
					},
					{
						id: 7,
						min: 500,
						max: 0
					}
				],
				rentData: [{
						id: 1,
						min: 0,
						max: 0,
						title: '不限'
					},
					{
						id: 2,
						min: 0,
						max: 2000,
						title: '2k以下/月'
					},
					{
						id: 3,
						min: 2000,
						max: 5000,
						title: '2k-5k/月'
					},
					{
						id: 4,
						min: 5000,
						max: 10000,
						title: '5k-10k/月'
					},
					{
						id: 5,
						min: 10000,
						max: 20000,
						title: '10k-20k/月'
					},
					{
						id: 6,
						min: 20000,
						max: 50000,
						title: '20k-50k/月'
					},
					{
						id: 7,
						min: 50000,
						max: 0,
						title: '50k以上/月'
					}
				],
				choseeRentInfo: {
					id: 1,
					min: 0,
					max: 0,
					title: '不限'
				},
				seekingData: [],
				loadMoreText: "加载中...",
				showLoadMore: false
			}
		},
		components: {
			uniIcon,
			uniPopup
		},  
		methods: {
			getRegionData() {
				let _this = this;
				uni.showLoading({
					title: '正在加载'
				});
				_this.$Request.get(_this.$api.config.region, {
					city: city
				}).then(res => {
					uni.hideLoading();
					if (res.code == 200) {
						_this.region = res.data;
					} else {
						uni.showModal({
							title: '温馨提示',
							content: res.msg,
							success(res) {
								uni.navigateBack();
							}
						})
					}
				});
			},
			togglePopup(type) {
				this.type = type;
			},
			// 行业选择
			cateChoose(type, cateInfo) {
				let _this = this;
				_this.choseeCateInfo = cateInfo;
				if (type == 1) {
					_this.topCateId = cateInfo.id;
					if (cateInfo.child) {
						_this.industryCateChild = _this.$util.objectConverArray(cateInfo.child);
					} else {
						_this.industryCateChild = [];
					}
					if (_this.topCateId == 0) {
						_this.getSeekingInfo(true);
					}
					if (_this.industryCateChild.length == 0) {
						_this.type = 'hide';
						_this.getSeekingInfo(true);
					}
				} else {
					_this.type = 'hide';
					_this.getSeekingInfo(true);
				}
			},
			// 地区选择
			regionChoose(type, areaInfo) {
				let _this = this;
				if (type == 1) {
					if (areaInfo.id != _this.district) {
						_this.twon = 0;
					}
					_this.district = areaInfo.id;
					if (_this.district == 0 || areaInfo.child.length <= 0) {
						_this.type = 'hide';
						_this.getSeekingInfo(true);
					}
				} else {
					_this.twon = areaInfo.id;
					_this.type = 'hide';
					_this.getSeekingInfo(true);
				}
			},
			// 面积选择
			areaChoose(areaInfo) {
				let _this = this;
				_this.choseeAreaInfo = areaInfo;
				_this.type = 'hide';
				_this.getSeekingInfo(true);
			},
			// 租金选择
			rentChoose(rentInfo) {
				let _this = this;
				_this.choseeRentInfo = rentInfo;
				_this.type = 'hide';
				_this.getSeekingInfo(true);
			},
			getSeekingInfo(isReload) {
				if (isReload) {
					uni.pageScrollTo({
						scrollTop: 0,
						duration: 300
					});
					is_load_more = true;
					page = 0;
				}
				let _this = this;
				_this.$Request.get(_this.$api.transferShop.seekingList, {
					city: city,
					district: _this.district,
					twon: _this.twon,
					area_min: _this.choseeAreaInfo.min,
					area_max: _this.choseeAreaInfo.max,
					rent_min: _this.choseeRentInfo.min,
					rent_max: _this.choseeRentInfo.max,
					cate_id: (_this.choseeCateInfo.id > 0 ? _this.choseeCateInfo.id : _this.topCateId),
					page: page,
					limit: limit
				}).then(res => {
					if (res.code == 200) {
						if (isReload) {
							_this.seekingData = [];
						}
						for (let i in res.data) {
							_this.seekingData.push(res.data[i]);
						}
						if (res.data.length < limit) {
							_this.loadMoreText = "没有更多数据了!"
							is_load_more = false;
						}
					}
					uni.stopPullDownRefresh();
					// console.log(res);
				});
			}
		},
		onShow() {
			let _this = this;
			var chosenRegion = _this.$SysCache.get('chosen_region');
			if (chosenRegion.value != city) {
				city = chosenRegion.value;
				_this.getSeekingInfo(true);
				_this.getRegionData();
			}
		},
		onLoad() {
			let _this = this;

			webConfig = _this.$SysCache.get('web_config');
			_this.industryCate = _this.$util.objectConverArray(webConfig.transferShop.industryCate);
			_this.industryCate.unshift({
				id: 0,
				name: '全部',
				child: {}
			});


			// me.getSeekingInfo();
		},
		onUnload() {
			this.max = 0,
				this.data = [],
				// this.loadMoreText = "加载更多",
				this.showLoadMore = false;
		},
		onReachBottom() {
			if (is_load_more) {
				page++;
				this.showLoadMore = true;
				this.getSeekingInfo();
				// console.log("onReachBottom");
			}
		},
		onPullDownRefresh() {
			this.loadMoreText = "加载中...";

			// console.log('onPullDownRefresh');
			// this.seekingData = [];
			this.getSeekingInfo(true);
		}
	}
</script>

<style>
	@import url("qnquiry.css");
</style>
