	...
	<view>
		 <view class="releabBox">
				...
				<view class="" v-for="(item,index) in imgs" :key="index">
					...
					<!-- <view class="releaimadlt" @click="uploadImg(item2.type, index2)" v-for="(item2,index2) in item" :key="index2"> -->
						<view class="releaimadcont">
							<view class="relecle" v-if="item2.is_close !== false"><uni-icon class="i" type="closeempty" color="#ffffff" size="18" @click="delImg(item2.type, index2)"></uni-icon></view>
			...
			<view class="releaitianjaBox">
				<view class="releaitianja" @click="uploadMoreImg"><text>添加更多图片</text><uni-icon type="plusempty" color="#666666" size="20"></uni-icon></view>
			</view>
	...
	export default {
		data() {
			return {
				imgs: {
					1:[
						{type: 1, title: '正面照', show_img: '', is_close: false},
					],
					2:[
						{type: 2, title: '左侧照', show_img: '', is_close: false},
					],
					3:[
						{type: 3, title: '右侧照', show_img: '', is_close: false},
					],
					4:[]
				}
			}
		...
		onLoad() {
			...
			for(var i in transferShopInfo.shop_img) {
				for(var j in transferShopInfo.shop_img[i]) {
					var arr = {src:transferShopInfo.shop_img[i][j].img_url};
					if(uploadImages[i]) {
					...
					if(imgs[i][j]) {
						...
						imgs[i][j] = {type: i, title: transferShopInfo.shop_img[i][j].type_name, show_img: transferShopInfo.shop_img[i][j].show_img_url};
					}
				}
			}
			me.imgs = [];
		...
		methods:{
			...
			addMore() {
				var me = this;
				me.imgs[4].push({type:4, title: '其他照', show_img:''});
			}
		}
	}
</script>
